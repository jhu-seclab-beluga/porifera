"""Tests for lemur._operations._instrumenter using real PHP code."""
...
def test_instrument_wraps_echo_string(tmp_path: Path):
    """Wrapping echo 'hello' produces __lemur_probe_xxx('key', 'hello')."""
...
def test_instrument_wraps_numeric_literal(tmp_path: Path):
    ...
    assert "__lemur_probe_abc('x_val', 42)" in output
...
def test_instrument_skips_lvalue(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_test")
...
def test_instrument_function_call_argument(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_arg")
    result = instrumenter.instrument_node(var.id, "a_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_arg('a_val'" in output
...
def test_instrument_binary_op_rhs(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_binop")
    result = instrumenter.instrument_node(rhs.id, "rhs_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_binop('rhs_val', 1)" in output
...
def test_instrument_ternary_branch(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_tern")
    result = instrumenter.instrument_node(yes_node.id, "yes_branch")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_tern('yes_branch'" in output
...
def test_instrument_wraps_method_call_result(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_method")
    result = instrumenter.instrument_node(method_call.id, "status_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_method('status_val'" in output
...
def test_instrument_wraps_static_method_call(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_static")
    result = instrumenter.instrument_node(static_call.id, "create_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_static('create_val'" in output
...
def test_instrument_wraps_property_fetch(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_prop")
    result = instrumenter.instrument_node(prop.id, "name_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_prop('name_val'" in output
...
def test_instrument_wraps_new_instance(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_new")
    result = instrumenter.instrument_node(new_expr.id, "new_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_new('new_val'" in output
...
def test_instrument_wraps_class_constant(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_const")
    result = instrumenter.instrument_node(const_fetch.id, "const_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_const('const_val'" in output
...
def test_instrument_wraps_null_coalesce(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_coal")
    result = instrumenter.instrument_node(coalesce.id, "coalesce_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_coal('coalesce_val'" in output
...
def test_instrument_wraps_cast_expression(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_cast")
    result = instrumenter.instrument_node(cast.id, "cast_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_cast('cast_val'" in output
...
def test_instrument_wraps_boolean_not(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_not")
    result = instrumenter.instrument_node(not_expr.id, "not_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_not('not_val'" in output
...
def test_instrument_wraps_instanceof(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_inst")
    result = instrumenter.instrument_node(instanceof.id, "inst_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_inst('inst_val'" in output
...
def test_instrument_wraps_string_concat(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_concat")
    result = instrumenter.instrument_node(concat.id, "concat_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_concat('concat_val'" in output
...
def test_instrument_wraps_array_literal(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_arr")
    result = instrumenter.instrument_node(array_expr.id, "arr_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_arr('arr_val'" in output
...
def test_instrument_wraps_closure(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_cls")
    result = instrumenter.instrument_node(closure.id, "closure_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_cls('closure_val'" in output
...
def test_instrument_wraps_arrow_function(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_arrow")
    result = instrumenter.instrument_node(arrow.id, "arrow_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_arrow('arrow_val'" in output
...
def test_instrument_wraps_inside_class_method(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_cls_m")
    result = instrumenter.instrument_node(plus.id, "add_result")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_cls_m('add_result'" in output
...
def test_instrument_wraps_inside_if_condition(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_if")
    result = instrumenter.instrument_node(func_call.id, "cond_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_if('cond_val'" in output
...
def test_instrument_wraps_inside_while_condition(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, StandardProbeStrategy(), "__lemur_probe_while")
    result = instrumenter.instrument_node(gt.id, "cond_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_while('cond_val'" in output
...
def test_instrument_elevating_wraps_post_increment(tmp_path: Path):
    ...
    instrumenter = ASTInstrumenter(ast, ElevatingProbeStrategy(), "__lemur_probe_elev")
    result = instrumenter.instrument_node(var.id, "inc_val")
    assert result is True

    output = PrettyPrinter().print_file(ast, ast.file_nodes()[0].get_property("relativePath"))
    assert "__lemur_probe_elev('inc_val'" in output
